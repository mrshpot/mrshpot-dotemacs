...
def lint(filename):
    ...
    # Start pylint
    executable = os.getenv("PYMACS_PYTHON")
    if not executable:
        raise ValueError("PYMACS_PYTHON is not set")
    cmdline = ' -c "import sys; from pylint import lint; lint.Run(sys.argv[1:]);" \
-f parseable -r n --disable=C,R,I "%s"'
    process = Popen(executable + cmdline %
